...
1. *Motivation*: absorbing the orphaned cost typically cannibalises the incumbent's existing margin structure, so the expected value of absorption is lower for the incumbent than for the entrant — Arrow's replacement effect applied to organisational strategy.[^16]
2. *Architecture*: the incumbent's organisation is itself an equilibrium — internal coordination, career incentives, and asset bases settled around the old boundary — so imitation requires terminating an internal equilibrium, which is exactly as hard as the theory predicts.
3. *Time*: by the moment imitation becomes obviously necessary, the absorber often holds network or scale positions that make late absorption non-equivalent. Adidas adopting the Nike model in 1995 is categorically different from Nike deploying it in 1975.
...
> Complex systems relax into constraints. Change the constraints that bind behaviour, and the system re-equilibrates. Leave constraints unchanged, and no amount of persuasion or incremental effort produces structural transformation.
